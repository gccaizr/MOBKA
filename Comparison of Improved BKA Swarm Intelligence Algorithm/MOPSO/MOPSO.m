...
% ----------------------------------------------------------------------- %
function f = MOPSO(params,MultiObj)
...
POS = zeros(Np,nVar);
% POS = initialize_variables(Np, numOfObj, nVar, var_min, var_max,fun);
for i=1:Np
    POS(i,:)=var_min+(var_max-var_min).*rand(1,nVar);
    POS_fit(i,1:M)  = fun(POS(i,:));
end
...
if(nmut>0)
    ...
    POS(idx,:) = var_min+(var_max-var_min).*rand(nmut,nVar);
end
