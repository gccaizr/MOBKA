...
function f = MSSA(params,MultiObj)
name=MultiObj.name;%问题名
obj_no=MultiObj.numOfObj;%目标函数个数
ObjectiveFunction=MultiObj.fun;
dim=MultiObj.nVar;
lb=MultiObj.var_min;
ub=MultiObj.var_max;
max_iter = params.maxgen;  % Set the maximum number of generation (GEN)
N = params.Np;      % Set the population size (Search Agent)
ishow = 1;
